...
const BudgetInfluencers = () => {
  const { data: creators = [], isLoading, error, refetch } = useQuery({
    queryKey: ['budget-creators'],
    queryFn: async () => {
      try {
        // Fetch creators with price less than $50 and their ratings
        const { data, error } = await supabase
          .from('creator_applications')
          .select('*, video_requests!creator_id(*)')
          .eq('status', 'approved')
          .lt('price', 50) // Get creators with price less than $50
        ...
        toast({
          title: "Error fetching budget creators",
          description: "There was a problem loading the budget creators.",
          variant: "destructive"
        });
        return [];
      }
  ...
  useEffect(() => {
    const channel = supabase
      .channel('budget-creators-changes')
      .on(
        'postgres_changes',
        {
          event: '*',
    ...
    <section className="py-12 bg-gray-50">
      <div className="section-container">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold">Creators Under $50</h2>
          <Link to="/budget-influencers" className="hidden md:block">
...
export default BudgetInfluencers;
